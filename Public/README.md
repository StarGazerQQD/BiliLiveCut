...
**版本：V0.1.5.1 Alpha** (`0.1.5.1-alpha`)

BiliLiveCut 是一个**全自动 AI 直播切片系统**：监听 Bilibili 直播间 → 实时录制 + 转写 → 识别高光爆点 → 生成剪辑成品 + 文案。

这个 `Public/` 目录是**即插即用分发版**：目标电脑只需装好 Python 3.11+，**双击 `launcher.exe`**，剩下的一切（源码、依赖、模型、FFmpeg）自动下载，直接跑到网页控制台。
...
打包完成后把整个 `Public/` 目录拷到目标机，双击 `launcher.exe` 即可**离线启动**。
...
此 `Public/` 目录是**发布给最终用户的即插即用版本**。如果你需要参与开发或定制，请回到上层目录查看主工程 `README.md` 及 `CHANGELOG.md`。
