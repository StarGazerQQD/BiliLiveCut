...
在一台**能联网**的机器上运行一次,把「模型 + 依赖 + 源码」全部封装进 ``Public/``:

1. **模型**:下载 faster-whisper 的 ``large-v3-turbo``(Systran 转换版)到
   ``Public/models/whisper-large-v3-turbo``(境内默认走 hf-mirror.com 镜像);
2. **依赖**:按 ``requirements-bundle.txt`` 把全部 wheel 下载到 ``Public/vendor/wheels``
   (供离线 ``install`` 使用,封装所有外部库);
3. **源码**:把主工程的 ``app/`` ``config/`` 等复制进 ``Public/``,使其自包含。

打包完成后,把整个 ``Public/`` 目录拷到目标机器,依次运行 ``install`` 与 ``run`` 即可
...
# 注意:主工程 README.md 会被复制为 README_MAIN.md,避免覆盖 Public 自己的说明。
...
def copy_source() -> None:
    ...
    for name in SOURCE_ITEMS:
        ...
        log(f"已复制源码:{name}")
    # 主工程 README 另存为 README_MAIN.md,避免覆盖 Public 说明。
    main_readme = PROJECT_ROOT / "README.md"
    if main_readme.exists():
        shutil.copy2(main_readme, PUBLIC_DIR / "README_MAIN.md")
        log("已复制源码:README.md -> README_MAIN.md")


def download_model(hf_mirror: str) -> None:
    """下载 large-v3-turbo 模型到 Public/models。

    :param hf_mirror: HuggingFace 镜像地址(境内建议 https://hf-mirror.com)。
    """
    target = PUBLIC_DIR / "models" / MODEL_DIRNAME
...
def vendor_wheels(pip_index: str) -> None:
    ...
    """
    wheels = PUBLIC_DIR / "vendor" / "wheels"
    wheels.mkdir(parents=True, exist_ok=True)
    req = PUBLIC_DIR / "requirements-bundle.txt"
...
def collect_status(build_platform: dict | None = None) -> dict:
    ...
    model_dir = PUBLIC_DIR / "models" / MODEL_DIRNAME
    ...
    wheels_dir = PUBLIC_DIR / "vendor" / "wheels"
    ...
    src_items = {name: (PUBLIC_DIR / name).exists() for name in SOURCE_ITEMS}
    source = {"items": src_items, "ok": all(src_items.values())}
    env_present = (PUBLIC_DIR / ".env").exists()
...
def repair(args: argparse.Namespace) -> bool:
    ...
    * 缺源码 → 尝试从主工程复制。
    ...
    if platform_mismatch:
        ...
        purged = _purge_dir(PUBLIC_DIR / "vendor" / "wheels", ["*.whl", "*.tar.gz", "*.zip"])
        purged += _purge_dir(PUBLIC_DIR / "bin", ["ffmpeg*", "ffprobe*"])
