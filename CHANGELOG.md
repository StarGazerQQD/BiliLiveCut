...
## V0.1.12.1-HL-Alpha (2026-07-06)

### 同步 main v0.1.12.1 + 自有 HL 模型

- **合并母仓库 main**: v0.1.12.1-alpha (ebea663)，含多引擎 ASR 流水线重构、数据一致性与管线稳定性增强、审计安全加固
- **HL 独有 feature**: 98维特征提取 + XGBoost 模型 + SelfLearnEngine + ModelRegistry + DriftDetector + Web ML仪表盘 + CLI 审计
- **C 加速**: fast_multi_emotion + fast_sliding_max + fast_count_bursts（HL 专属 3 函数）+ main Rust/Cython 第二轮加速

## V0.1.12.1 Alpha (2026-07-05)
