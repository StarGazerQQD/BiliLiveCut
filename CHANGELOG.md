...
- **highlight-ml**: 新增单一 JSON 模型产物、SHA-256 原子注册、Champion/Shadow、真实回滚、generation 热加载，以及基于真实训练分布的 PSI/均值/缺失率漂移检测。
- **highlight-ml**: 接入主程序 Worker 与同步评分入口，新增默认关闭的 off/shadow/champion 模式、房间覆盖、规则回退、SystemLog 预测审计，以及 Web/CLI 状态入口。
- **highlight-ml**: 补齐真实数据库训练注册、原子漂移/盲审附件、Shadow 设置/晋升/回滚 CLI、基于线上日志的漂移报告和 Web 运维展示。

### 修复

- **review/clipping**: 审片边界调整改为接收前端 JSON 并校验起止关系、时长、录像范围和分片缺口；重新渲染显式传递人工边界并使用独立版本文件，修复未提交临时边界对渲染线程不可见以及重复渲染覆盖旧成片的问题。
- **review**: 审片数据恢复已保存的人工边界，并修复无弹幕时查询条件退化为裸时间值导致页面 500 的问题。
- **recording**: 增加 starting/stopping/finalizing/paused 生命周期、优雅/强制停止、暂停后新会话恢复及可选取消下游任务；人工停止会持久化暂停状态，防止 `auto_record` 立即重新拉起。
- **recording**: 增加直播中人工高光打点，支持前后缓冲和备注；会话结束时自动把候选窗口收敛到真实录像范围，无媒体或过短片段进入上下文不足审核状态。
- **pipeline**: 任务取消允许从任意非终态进入 `cancelled`，并统一处理 SQLite UTC-naive 时间戳的耗时计算。
- **rooms**: 修复 `room_config` API 错写不存在的 ORM 属性，并改为合并更新 `room_config_json`，防止局部设置清除已有热词等配置。
- **review**: 新增独立审核员账号和最小权限隔离、人工审核队列、带有效期的互斥领取及管理员强制接管，线上审核不再依赖共享管理员账号。
- **review**: 新增盲审、持久化草稿、边界/决策撤销及 `SystemLog` 结构化审计；审核提交后自动释放领取，其他审核员草稿不会互相泄露。
- **review**: 审片播放器改用受控媒体 API，不再把本机成片路径直接作为浏览器视频地址。
- **web/jobs**: 新增持久化后台作业管理器和 `/api/jobs` 查询、取消、重试接口；候选出片、审片重渲染、合集渲染及上传操作改为立即返回作业，支持去重、进度、错误和重启恢复。
- **render**: 主剪辑、派生版本和合集 FFmpeg 改为可协作取消的子进程执行，取消时主动终止当前命令并清理未完成输出；审核重渲染不再额外生成或覆盖派生版本。
- **upload**: 已开始的上传明确禁止不安全取消；服务中断后标记为需核对结果而不自动重放，降低重复投稿风险。
- **web/ui**: 重做控制台、人工审核队列、审片工作台和合集编辑器的统一视觉系统；主导航按录制/制作/配置/系统分组，补齐响应式布局、清晰焦点态、减少动态效果偏好及静态资源缓存版本。
- **web/ui**: 修复候选拒绝模块模板字符串未闭合导致控制台业务脚本无法加载，以及审核/合集页面使用旧版 `TemplateResponse` 签名导致当前 Starlette 下返回 500 的问题。
