...
## V0.1.10.1-HL-Alpha (2026-07-05)

### 同步 main v0.1.10.1 + 自有 HL 模型

- **合并母仓库 main**: v0.1.10.1-alpha (0610bdf)，含第二轮 Rust/Cython 加速（聚类矩阵+弹幕基线+SRT组装）、安全加固、全量审计修复
- **HL 独有 feature**: 98维特征提取 + XGBoost 模型 + SelfLearnEngine + ModelRegistry + DriftDetector + Web ML仪表盘 + CLI 审计
- **C 加速**: fast_multi_emotion + fast_sliding_max + fast_count_bursts（HL 专属 3 函数）
