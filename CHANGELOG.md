...
## V0.1.18.4 Alpha (2026-09-22)

- macOS CI 覆盖率测试时限扩至 90 分钟，任务总时限设为 120 分钟，为依赖安装和报告上传保留余量，避免接近完成时被旧的 45 分钟上限中断。
- 限定 SQLModel 的兼容版本范围，保持既有数据库无时区 UTC 语义；Windows CI 直接使用真实 FFmpeg/FFprobe 二进制，避免 Chocolatey 包装进程导致停录验收超时。
- Rust 编译统一映射本机源码与依赖缓存目录，Payload 拒绝包含本机路径的原生扩展，避免将构建用户目录带入分发包。
- 修复来源认证等永久取流错误被开播监控反复拉起的问题；保留自动重启限制，处理后显式启动或重新开启自动录制即可恢复。
- 通过独立示例及本地 HTTP/真实 FFmpeg 验证直播源录制、重连、持久任务、无弹幕分析与出片；同步 wheel/sdist/Portable 的契约文档及示例收录规则。
- 修复晚确认热点补评分后候选未关联处理任务的问题，在同一分析租约下原子提交候选和任务承接；保留人工审核与上传开关。
- 新增版本为 `1` 的正式直播源插件契约、来源注册及安全停用，复用既有插件设置与生命周期。
- 房间以平台和稳定字符串 ID 关联，Web、CLI、预约、开播监控、标题刷新、录制和恢复共用来源接口；Schema 保持 v5。
- 重连重新取流，播放凭据不落普通会话字段或日志；弹幕按场次真实连接区间记录，缺失证据不参与数值评分，保留 Bilibili 与本地录播语义。
- 版本升级至 `0.1.18.4-alpha`（标签 `v0.1.18.4-Alpha`），同步原生扩展版本、Portable 源码冻结及分发信息。沿用应用版本严格校验；升级需保留旧目录并创建新版数据库，不自动迁移或修改旧数据。
