...
适用主程序：`V0.1.18.0 Alpha`；当前插件 API 版本仍为 `1`。Event-first 热点召回、跨分段事件生命周期、事件补全与成片评分由宿主管理，没有改变插件清单、生命周期或 `HighlightScoringRequest` 契约；既有评分插件继续作用于宿主生成的候选，不负责创建或持久化 `HotspotEvent`。
