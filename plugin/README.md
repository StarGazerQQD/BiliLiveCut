...
适用主程序：`V0.1.17.2 Alpha`；当前插件 API 版本仍为 `1`。场次时间线、多峰值、跨分段动态边界、弹幕延迟校正与重分析均由宿主管理，没有改变插件清单、生命周期或 `HighlightScoringRequest` 契约；插件仍接收完整原始分片上下文，宿主按独立音频峰值候选窗口计算规则特征和最终 LLM 理由。
