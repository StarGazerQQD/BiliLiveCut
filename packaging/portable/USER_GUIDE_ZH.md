...
适用版本：`v0.1.17.4-Alpha` · 适用系统：Windows 10/11 x64
...
- 服务启动后按黑色窗口打印的地址访问；首次默认是 <http://127.0.0.1:8000>。当前版本不保证自动打开浏览器。
...
## V0.1.17.4 Alpha 先知道的新流程

- “候选审核”首页升级为“场次时间线”。每场录制按 GMT+8 展示高光节点、摘要、1～2 条代表弹幕、置信度和来源依据；整场结束并完成最终分析后，系统会按时间顺序汇集同场全部最终 ASR，并把完整上下文一次性交给 LLM 做全局分析，生成不按五分钟录制分片拆开的全场高光总结。点击“审片”仍可播放、调整边界、批准或拒绝。
...
- “实时转写”和“弹幕热度”都可用“录制场次”下拉查看数据库中的完整历史；新增直播不会把旧转写挤掉，两个页面会分别记住当前选择。“实时转写”还可以人工改正文，并可填写“错误词=正确词”加入该直播间的专属词典；每条记录会显示并可复制对应的源 TS 文件名，也可“无损导出 MP4”直接进入剪辑软件。导出保留原始编码并校准首个视频帧到 `0s`，不会因 AAC 预滚在开头留下黑帧；保存纠错后可自动重分析本场。
...
- 五秒轮询会保留转写纠错、房间词典、房间配置、功能开关和下拉选择中的未保存内容，也会记住原始 ASR、已学习词典及时间线评分详情的展开状态。转写编辑器展开或有未保存内容时，“录制场次”下拉会锁定，保存或取消后才可切换。时间线内容没有变化时不会重建页面；出现新节点或状态变化时也会恢复原滚动位置，阅读摘要或来源评分时不会自动跳到顶部。
...
打开项目的 [GitHub Releases 页面](https://github.com/StarGazerQQD/BiliLiveCut/releases)，进入 `v0.1.17.4-Alpha`，下载：

1. `BiliLiveCut-Portable-Full-0.1.17.4-alpha-x64.zip`
2. `SHA256SUMS.txt`
...
```powershell
Get-FileHash ".\BiliLiveCut-Portable-Full-0.1.17.4-alpha-x64.zip" -Algorithm SHA256
```
...
2. 右键 ZIP，选择“全部解压”。
3. 打开解压出来的 `BiliLiveCut-Portable-Full-0.1.17.4-alpha-x64` 文件夹。
4. 确认同一层能看到：
...
```text
BiliLiveCut-EnginePack-0.1.17.4-alpha.zip
```
...
5. 出现类似下面的文字后，服务已经启动。若你改过 Web 端口，以黑色窗口实际打印的地址为准：
...
1. 打开“配置 → 功能开关”，找到“Web 管理端口”。
2. 输入 `1` 到 `65535` 之间的整数并保存。页面会分别显示“已保存端口”和“当前监听端口”。
3. 保存不会中断或热重绑当前服务；先停止 Launcher，再重新双击启动。
4. 重启后黑色窗口和页面中的当前端口都会变为新值，`需要重启` 提示消失。

端口保存在程序根目录 `config\launcher.json`，业务 Runtime 更新和修复不会覆盖它。不要手工写入无效 JSON；若文件已损坏，Launcher 会告警并回退到 `8000`，但为保留排障现场不会自动覆盖原文件。端口已被其他程序占用时，Launcher 会明确指出端口并停止，不会随机换端口。
...
- 高光默认初筛/候选/人工审核阈值为 `0.28/0.38/0.32`，自动批准/发布阈值为 `0.72/0.80`。阈值越低越容易产生候选；房间级值可在“功能开关”中调整。
...
- Web「配置 → 大模型」没有启用服务商：转写梳理自动跳过，高光判断使用本地规则评分，不产生 API 费用；整场 ASR 全局分析会明确显示不可用。
- `TREND_ENABLED=false`：不启用联网热点采集。
- `UPLOADER=manual`：只生成本地文件，不自动投稿。

若当前场次出现明显重复转写，可在“实时转写”页点击“重新识别”。该操作会重建本场自动分析结果；有活动任务时会等待或显示明确原因。
...
1. 保持黑色 Launcher 窗口开启，并打开窗口实际打印的 `http://127.0.0.1:<端口>`；首次默认端口为 `8000`。
2. 进入“直播间”页面。
...
8. 在房间卡片中点击“开始录制”。
   程序会在真正创建本场录制前重新查询一次直播间标题和主播名，因此同一房间改标题后会使用最新资料；若平台详情接口暂时失败，则保留最近一次成功资料并继续录制。
...
- 首次测试可保持 `COLLECT_DANMAKU=true`，在“弹幕热度”页选择本次“录制场次”，确认已经收到数据。以后可从同一下拉切回任何仍保存在数据库中的旧场次。
...
2. 录制结束且最终分析稳定后，展开场次即可在节点上方阅读“全场高光总结”；它把整场最终 ASR 按 GMT+8 时间顺序组成一个上下文，交给 LLM 一次完成全局分析，不拼接节点摘要，也不按 TS/五分钟分片拆段。人工纠错或重分析后旧总结会自动失效并重新排队；也可点击“重新生成”。LLM 没有返回有效正文时任务会明确失败并重试，不会用规则文本冒充分析结果。
3. 展开场次卡片查看高光节点。每个节点提供时刻、摘要、最多两条代表弹幕、置信度、来源信号以及动态/跨分段标记。
4. 点击节点中的“审片”播放候选；确认内容后点击“批准并出片”，不需要的候选点击“拒绝”。
5. 默认不显示已拒绝节点；需要核查时打开“包含已拒绝”。
6. 在“成品切片”中播放并检查成品。“发布（置 ready）”只会标记本地交付状态并导出清单，默认不会自动投稿。
7. 在“上传 / 设置”中点击“打开切片目录”。

需要重算时：

- 只改了审核阈值，点击场次卡片的“按新阈值重分析”。
- 改了房间手工词典、人工学习别名或 ASR 模型，点击“按新词典/模型重新转写”。
- 在“实时转写”先从下拉选择录制场次，再编辑该场正文。可额外输入若干行 `错误词=正确词`，勾选加入直播间词典；编辑器展开或存在未保存内容时，自动刷新不会覆盖草稿，场次下拉也会保持锁定。
- 重分析会按当前模型、词典和阈值重建本场自动结果；正在运行的任务会阻止操作并显示原因。
...
- Alpha 当前版只处理由同版本创建的场次数据，不读取或迁移其他版本的数据库。
...
Alpha 版本之间不提供数据迁移或向前兼容。测试另一个版本时：

1. 把新版本解压到全新目录。
2. 不要复制其他版本的 `.env`、`storage\`、`.venv`、`runtime`、`models\` 或 `vendor\wheels`。
3. 按新版本说明重新配置并完成基础自检。
4. 需要保留的原始录像或成片可作为普通媒体文件另行归档，不要把旧数据库放入新目录。
...
保持黑色窗口开启，按窗口打印的 `http://127.0.0.1:<端口>` 地址手动访问；未修改时端口为 `8000`。

### 浏览器显示“无法访问此网站”

- 检查黑色窗口是否仍在运行。
- 等待模型准备和 `[6/6] Starting Web console` 完成。
- 如果提示某个端口被占用，先关闭之前启动的 BiliLiveCut 或占用该端口的程序；也可以在控制台保存另一个端口后重启。程序不会自动随机换端口。
...
`v0.1.17.4-Alpha` 已把 OpenAI 兼容 SDK 纳入 Portable 运行时。该提示说明当前 Full 的 `vendor\wheels\` 不完整或目录不是本版本的干净发行包；请重新下载并解压到全新目录。不要在 Portable 目录手工执行 `pip install -e`。
...
新版 Full 应强制使用本地 wheelhouse，不应访问 PyPI 镜像。确认使用的是 `v0.1.17.4-Alpha` 最新 Full ZIP，并且没有只复制 EXE。不要修改锁文件或添加报错中的 sdist 哈希，直接重新下载并校验 Full ZIP。
...
V0.1.17.4 Alpha 会先按 `LIVE_OFFLINE_CONFIRM_COUNT` 连续确认下播，再等待 `LIVE_SESSION_END_DELAY_S` 秒；期间恢复开播会取消停止。断流恢复受 20 次或 180 秒预算限制，单场还受 `RECORDING_MAX_DURATION_S` 限制。预算耗尽后当前会话会停止，并等待房间至少一次真实离线后才允许下一次开播自动录制；最后一个录制任务结束时网感定时采集会恢复。

### 候选理由、预览和视频内容不一致

先确认使用 V0.1.17.4 Alpha。新分析会用同一个候选窗口生成规则特征、LLM 理由和动态边界，并强制成片覆盖完整窗口。需要重新计算当前场次时，在“场次时间线”执行重分析；改过词典或 ASR 时选择重新转写。受保护数据不会被静默改写。

### 拒绝候选后仍在成品列表显示 `reviewing`

V0.1.17.4 Alpha 的拒绝操作会同步拒绝当前候选、关联事件、任务和未发布成片，并默认从时间线过滤。已经发布的成片按设计保留，避免篡改真实外部发布结果。当前 Alpha 不读取其他版本的数据库；若启动时报告 Schema 或版本不匹配，请使用全新目录重新测试。
...
`v0.1.17.4-Alpha` 适合小规模、受控测试，不等同于稳定正式版。当前应重点验证：
