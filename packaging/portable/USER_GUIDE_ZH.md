...
适用版本：`v0.1.18.2-Alpha` · 适用系统：Windows 10/11 x64

这份说明面向不懂 Python、Git 或命令行的普通 Windows 用户。按顺序操作即可完成下载安装、首次启动、基础配置、添加直播间和首次录制。

需要了解程序架构、构建或版本变化时，请查看[项目 README](../../README.md)、[Portable 构建说明](README.md)、[当前 Changelog](../../CHANGELOG.md)和[历史归档索引](../../docs/changelog/CHANGELOG_INDEX.md)。V0.1.17～V0.1.17.4 的逐项记录位于[独立归档](../../docs/changelog/CHANGELOG_PRE_0.1.17.md)。
...
- Full **不包含语音模型**。如果没有内容身份兼容的 Engine Pack 或既有 `models/`，首次启动仍需联网下载约 5.5 GB 模型。
...
- 热点由弹幕、音频、SenseVoice、ASR 和趋势共同召回；ASR 不可用或质量不足时不会停掉整条高光流水线。事件可跨连续 TS 分段合并，一个分段也可有多个无关事件；真实断流缺口不可跨越，成片边界按完整事件和前后文动态计算。
- 整场结束并完成最终分析后，系统会把同场全部最终 ASR 按时间顺序组成一个上下文，交给 LLM 做一次全局分析，生成不按五分钟分片拆开的全场高光总结；这不是把节点摘要直接拼起来。
- Portable 首次在线准备模型时会使用已经装好依赖的 `.venv` Python。程序托管的 `.venv` 损坏会自动重建，依赖或单个模型下载中断可在下次启动续跑；真实 Python 3.10/3.13/3.14 等不支持版本会明确拒绝且不会被误删。
- Engine Pack ZIP 只接受 schema 5 内容清单，安装后的 `models/` 只接受 schema 6 清单。四个引擎按各自不可变来源与内容指纹独立校验；全部未变时不会下载，只有一个变化时只更新一个。其他 schema 不会被迁移或补写。
- 当前 Payload 内置 C、Cython、Rust 三个 `app.accelerators` 原生模块：候选聚类和弹幕文本特征使用 Rust/rayon，音频峰值、静音区间和热点滚动基线使用 Cython；不提供旧模块路径。源码环境没有编译扩展时会按函数使用等价 Python 实现。
- 设置页的“Web 服务端口”保存的是下次启动值；当前服务不会重新绑定。重启后 Launcher 读取程序根目录 `config\launcher.json`，默认仍为 `8000`，监听地址固定为 `127.0.0.1`。
...
打开项目的 [GitHub Releases 页面](https://github.com/StarGazerQQD/BiliLiveCut/releases)，进入 `v0.1.18.2-Alpha`，下载：

1. `BiliLiveCut-Portable-Full-0.1.18.2-alpha-x64.zip`
2. `SHA256SUMS.txt`
...
```powershell
Get-FileHash ".\BiliLiveCut-Portable-Full-0.1.18.2-alpha-x64.zip" -Algorithm SHA256
```
...
2. 右键 ZIP，选择“全部解压”。
3. 打开解压出来的 `BiliLiveCut-Portable-Full-0.1.18.2-alpha-x64` 文件夹。
4. 确认同一层能看到：
...
不要从不明网盘或陌生人处下载模型包。Engine Pack 校验失败时，程序会拒绝使用该包、保留已经验证的既有模型，并只在线准备缺失或身份变化的引擎。
...
首次启动后，可在“设置中心”的 ASR 分类直接修改设备、计算类型及并发；上面的环境配置作为部署默认。先证明录制链路正常，再单独测试 GPU。启用 CUDA 后可逐步提高“ASR 分段并行数”；实际模型由共享池按角色并发上限分配，过高的模型并发仍可能导致显存不足。页面标明下一任务或下一检查生效，已有任务保留启动快照。
...
5. 点击“添加”。
6. 打开“设置中心”→“房间自动化与端口”，找到刚添加的直播间；首次测试可只开启“自动分析”，确认录制和分析后再逐项开启自动渲染、自动审核与自动上传。
7. 点击“保存本直播间开关”。预约录制、阈值自学习、弹幕情绪、审核阈值和 ASR 手工词典也在这里按直播间独立设置；热词一行一个。
8. 在房间卡片中点击“开始录制”。
   程序在开录前查询标题与主播名，录制中默认每 30 秒刷新，并保存场次标题历史。接口失败时保留缓存、显示上次获取状态并继续录制。需要守候开播时，可在房间卡片选择“开播后自动录制并分析”，启用两个开关并解除暂停；Launcher 必须保持运行。
...
> “房间自动化与端口”中的流水线设置按直播间独立生效。关闭某一项只会阻止后续自动推进，仍可手动处理；自动投稿同时要求房间开关、设置中心的全局自动上传和上传工具开关开启，且候选分数达到房间投稿阈值。
...
- 点击“设置中心”→“账号登录”→“登录”后，程序会优先在独立临时资料目录中打开电脑已安装的 Google Chrome，并保持浏览器 sandbox 开启；它不会读取你的日常 Chrome Profile。
...
分析流水线只要检测到热点事件，场次时间线就会出现数据；是否生成视频候选由后续成片分独立决定：
...
3. 展开场次卡片查看热点节点。每个节点提供时刻、标题、摘要、最多两条代表弹幕、热度、成片分、语义置信度、证据覆盖和来源信号。黄色节点标记“仅时间线，不生成视频”，仍可用于回看整场发生了什么。
4. 达到房间成片阈值的节点会显示候选编号和“精审/预览”；确认内容后点击“批准并出片”，不需要的候选点击“拒绝”。没有候选编号的热点无需审核。
...
- 事件标题与摘要来自带 ID 的弹幕、ASR、音频等证据束；成片边界从完整事件起止点增加前后文，再向外吸附自然静音并限制在同一连续录像块内。播放器预览和最终成片使用同一候选边界，不会跨断流缺口。
...
V0.1.18.2 Alpha 只接受由当前版本创建并精确通过校验的数据库。V0.1.17.x 及其他历史数据库不会被备份、迁移或补写；切换版本时必须使用独立目录和新数据库。原始录像、成片与人工导出的资料可作为普通文件另行归档。历史版本的功能和修复记录只用于查阅，见 [0.1.17 系列 Changelog](../../docs/changelog/CHANGELOG_PRE_0.1.17.md)，不能当作当前数据迁移说明。

测试其他版本或需要保留独立环境时：

1. 把新版本解压到全新目录。
2. 不要复制其他版本的 `.venv`、`runtime` 或 `vendor\wheels`；这些运行资产必须由对应版本重新准备。
3. 只复制由当前版本生成的 `models\engine-pack-installed.json` 与对应模型目录；旧清单不会被自动升级。
4. 不要把其他版本的 `.env` 或 `storage\blc.db` 放入当前目录；按当前说明重新配置并创建新数据库。
5. 按新版本说明完成基础自检；原始录像或成片也可作为普通媒体文件另行归档。
...
`v0.1.18.2-Alpha` 已把 OpenAI 兼容 SDK 纳入 Portable 运行时。该提示说明当前 Full 的 `vendor\wheels\` 不完整或目录不是本版本的干净发行包；请重新下载并解压到全新目录。不要在 Portable 目录手工执行 `pip install -e`。
...
新版 Full 应强制使用本地 wheelhouse，不应访问 PyPI 镜像。确认使用的是 `v0.1.18.2-Alpha` 最新 Full ZIP，并且没有只复制 EXE。不要修改锁文件或添加报错中的 sdist 哈希，直接重新下载并校验 Full ZIP。

### 模型下载很慢或中断

- 保持磁盘空间和网络稳定。
- 关闭 Launcher 后重新双击，已有有效文件会尽量复用。
- 如果分发者提供经过校验且模型内容身份兼容的 Engine Pack，可放到 EXE 同级目录后重试；应用版本号不需要与包名相同。
...
- 使用完整直播间链接或正确房间号。
- 首次测试选择当前正在直播的房间。
- 原画不可用时，在“设置中心”搜索 `STREAM_QUALITY`，改为 `400` 或 `250` 并保存，下一场录制生效。

### 能录制但没有实时转写、候选或成片

先检查 `storage\raw\session_<数字>\` 是否已有 `.ts` 文件，再打开“设置中心”→“房间自动化与端口”核对该直播间是否启用了“自动分析”；需要自动成片时还要启用“自动渲染”。原始录制成功与后续自动阶段是否启用是两件事。请在问题报告中注明开关状态，不要反复删除整个程序目录。

### 主播下播后任务一直显示重连

V0.1.18.2 Alpha 会先按 `LIVE_OFFLINE_CONFIRM_COUNT` 连续确认下播，再等待 `LIVE_SESSION_END_DELAY_S` 秒；期间恢复开播会取消停止。断流恢复受 20 次或 180 秒预算限制，单场还受 `RECORDING_MAX_DURATION_S` 限制。预算耗尽后当前会话会停止，并等待房间至少一次真实离线后才允许下一次开播自动录制；最后一个录制任务结束时网感定时采集会恢复。

### 候选理由、预览和视频内容不一致

先确认使用 V0.1.18.2 Alpha。新分析先把多源证据聚合成完整热点事件，再用同一事件生成语义、成片分和动态边界；预览与最终成片使用同一候选边界。需要重新计算当前场次时，在“场次时间线”执行重分析；改过词典或 ASR 时选择重新转写。受保护数据不会被静默改写。

### 拒绝候选后仍在成品列表显示 `reviewing`

V0.1.18.2 Alpha 的拒绝操作会同步拒绝当前候选、关联事件、任务和未发布成片，并默认从时间线过滤。已经发布的成片按设计保留，避免篡改真实外部发布结果。任何非当前精确 Schema 的数据库都会明确拒绝，不会迁移或静默改写。

### 修改 `.env` 后没有生效

业务参数优先在“设置中心”按中文名称或 ENV 键搜索并保存，界面显示来源、生效时机和部署只读原因。数据库中的 Web 覆盖优先于 `.env`，如需重新使用环境值，先点击该字段“恢复默认”。`.env` 作为启动基线在服务启动时读取，修改它后需要关闭并重新启动 Launcher；凭据“明确清空”会阻止回退到环境旧值。
...
`v0.1.18.2-Alpha` 适合小规模、受控测试，不等同于稳定正式版。当前应重点验证：
