...
- “候选审核”首页升级为“场次时间线”。每场录制按 GMT+8 展示热点事件、摘要、1～2 条代表弹幕、热度、成片分和来源依据；未达到成片阈值的事件仍留在时间线上，但不会出现“精审/预览”入口。整场结束并完成最终分析后，系统会按时间顺序汇集同场全部最终 ASR，并把完整上下文一次性交给 LLM 做全局分析，生成不按五分钟录制分片拆开的全场高光总结。达到阈值的候选仍可点击“精审/预览”调整边界、批准或拒绝。
