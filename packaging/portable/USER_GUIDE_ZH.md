...
- Engine Pack 和 `models/` 使用当前 schema-6 内容清单。四个引擎按各自不可变来源与内容指纹独立校验；全部未变时不会下载，只有一个变化时只更新一个。旧版已安装清单不会被迁移或补写。
...
V0.1.18.0 Alpha 只接受由当前版本创建并精确通过校验的数据库。历史数据库不会被备份、迁移或补写；切换版本时必须使用独立目录和新数据库。原始录像、成片与人工导出的资料可作为普通文件另行归档。
...
V0.1.18.0 Alpha 的拒绝操作会同步拒绝当前候选、关联事件、任务和未发布成片，并默认从时间线过滤。已经发布的成片按设计保留，避免篡改真实外部发布结果。任何非当前精确 Schema 的数据库都会明确拒绝，不会迁移或静默改写。
