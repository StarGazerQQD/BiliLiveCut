...
**版本：V0.1.14.6 Alpha** (`0.1.14.6-alpha`)

BiliLiveCut 是一个**全自动 AI 直播切片系统**：监听 Bilibili 直播间 → 实时录制 + 转写 → 识别高光爆点 → 生成剪辑成品 + 文案。

这个 `packaging/portable/` 目录是**即插即用分发版**。`launcher.exe` 内嵌了**固定版本**的完整业务源码 (Commit `731a31c`，约 426KB Payload)，**双击即用，首次启动完全不访问 GitHub**。目标电脑只需装好 Python 3.11+，双击 `launcher.exe` 即可。
...
| 网络（安装） | **Lite**: 首次安装需联网拉取依赖；**Full**: 预置完整，安装无需额外下载；**模型**: 通过独立 **Engine Pack** 或在线下载安装 | — |
...
| 源码版本不确定（随 `main` 漂移） | 源码固定于 `731a31c`，SHA-256 可校验 |
...
| **安装过程** | 首次安装时自动联网下载 Python 依赖、FFmpeg | 预置全部组件，安装期间无需额外下载 |
| **模型** | 不含模型，通过独立 **Engine Pack** 或在线下载安装 | 不含模型，通过独立 **Engine Pack** 或在线下载安装 |
| **适用场景** | 日常使用，本地有 Python 环境 | 批量分发、网络受限的安装环境 |

> **模型策略**: Lite 和 Full 均不携带四引擎 ASR 模型。模型统一由独立的 **Portable Engine Pack** 提供。将 Engine Pack ZIP 放在程序同级目录，首次启动时自动校验 CRC32 并安装；无本地包时自动在线下载全部四个引擎模型。
...
1. 下载 BiliLiveCut-EnginePack-v0.1.14.6-alpha.zip
...
输出:
- dist/engine-pack/BiliLiveCut-EnginePack-v0.1.14.6-alpha.zip
- dist/engine-pack/engine-pack-manifest.json
- dist/engine-pack/CRC32SUMS.txt
- dist/engine-pack/SHA256SUMS.txt
- 
esources/engine_pack_info.json (自动供 Lite/Full EXE 嵌入)
...
> **源码固定**：源码来源固定为 Commit `731a31c`，不随 GitHub 上游变动。
...
├── build_payload.py                 # Payload 构建器 (731a31c → source_payload.zip)
...
> **源码去哪了？** `app/` `config/` `pyproject.toml` 等业务文件**不在分发目录中**，而是内嵌在 `launcher.exe` 内部作为 **source_payload.zip**（从 Commit `731a31c` 提取，固定版本，SHA-256 可校验）。`launcher.exe` 首次运行时自动将 Payload 解压到 `runtime/releases/` 目录。PnP 目录始终保持最小体积，源码不受 GitHub 主仓库 `main` 分支变动影响。
...
│   └── releases/
│       └── 0.1.14.6-alpha+731a31c/  # Payload 释放的固定版本源码
│
...
1. **双击 `launcher.exe`** → 等待自动部署完成（Lite 版需联网下载依赖和模型约 10-30 分钟；Full 版预置完整，秒开）
...
此 `packaging/portable/` 目录是**发布给最终用户的即插即用版本**，源码固定于 Commit `731a31c` (v0.1.14.6-alpha)。
