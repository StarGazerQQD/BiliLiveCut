...
**版本：V0.1.17.2 Alpha** (`0.1.17.2-alpha`)
...
1. 下载 BiliLiveCut-EnginePack-0.1.17.2-alpha.zip
...
- dist/engine-pack/BiliLiveCut-EnginePack-0.1.17.2-alpha.zip
...
│   └── releases/
│       └── 0.1.17.2-alpha+<source-sha>+<payload-hash>/  # 内容寻址的固定版本源码
│
...
| 时间线摘要与预览内容不一致 | 确认使用 V0.1.17.2 Alpha，并在场次卡片中按需执行“按新阈值重分析”或“按新词典/模型重新转写”；受保护资产不会被静默覆盖 |
...
此 `packaging/portable/` 目录是**发布给最终用户的即插即用版本**，源码固定于 `v0.1.17.2-Alpha` 的发布基线 Commit。
