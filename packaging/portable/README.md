...
**版本：V0.1.15 Alpha** (`0.1.15-alpha`)

BiliLiveCut 是一个**全自动 AI 直播切片系统**：监听 Bilibili 直播间 → 实时录制 + 转写 → 识别高光爆点 → 生成剪辑成品 + 文案。

这个 `packaging/portable/` 目录是**即插即用分发版**。`launcher.exe` 内嵌了当前发布基线的完整业务源码 (Commit `4bdaa13`)，**双击即用，首次启动完全不访问 GitHub**。目标电脑只需装好 Python 3.11+，双击 `launcher.exe` 即可。
...
1. 下载 BiliLiveCut-EnginePack-0.1.15-alpha.zip
...
输出:
- dist/engine-pack/BiliLiveCut-EnginePack-0.1.15-alpha.zip
...
│   └── releases/
│       └── 0.1.15-alpha+4bdaa13+<payload-hash>/  # 内容寻址的固定版本源码
│
...
此 `packaging/portable/` 目录是**发布给最终用户的即插即用版本**，源码固定于当前发布基线 Commit `4bdaa13` (`v0.1.15-alpha`)。
