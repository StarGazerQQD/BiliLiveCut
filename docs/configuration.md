...
- LLM 服务商：llm_providers 保存完整有序列表；按 ID 保留空白密钥，clear_api_key 明确清除，重复 ID 和非有限价格拒绝保存。连接缓存按服务商 ID 与地址索引，通过常量时间比较 SDK 已持有的完整凭据值判断能否复用；凭据及其摘要不进入缓存键，旧连接在活动调用结束后退役。
