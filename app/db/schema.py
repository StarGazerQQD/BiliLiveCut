...
* 数据库不存在时创建; 存在的数据库仅在精确匹配时启动;
* 任何校验失败均阻止应用启动。

Alpha 版本不提供历史数据库迁移或兼容接口。
"""
...
def _compute_model_schema_fingerprint() -> str:
    """按当前模型计算唯一受支持的 Schema 指纹。"""
...
def assure_schema() -> None:
    """确保数据库 Schema 可用 — 创建或精确校验。

    - 数据库不存在 → 创建全部表并写入 schema_meta
    - 数据库已存在 → 严格校验; 不兼容则抛出 RuntimeError
...
def _verify_critical_indexes() -> bool:
    ...
    try:
        with _get_engine().connect() as conn:
            ...
            indexed_columns = {
                tuple(str(column[2]) for column in conn.exec_driver_sql(f"PRAGMA index_info('{row[1]}')").fetchall())
                for row in rows
            }
