...
加速链 (优先级从高到低):
  第一轮 — Aho-Corasick + 余弦相似度 + bigram
    1. C 扩展 (_c_speedups) → 2. 纯 Python (python_fallback)

  第二轮 — 聚类矩阵 + 弹幕基线 + SRT 组装
...
try:
    ...
    _BACKEND = "python"
    _logger.info("加速模块(Python): 使用 accelerators.python_fallback.speedups")
...
try:
    ...
    from app.accelerators.python_fallback.speedups_round2 import (  # type: ignore[no-redef]
        danmaku_baseline_rate,
        group_srt_blocks,
    )

    _logger.info("加速模块(第二轮): 使用 accelerators.python_fallback.speedups_round2")
...
if _cluster_sim_func is None:
    try:
        ...
        from app.accelerators.python_fallback.speedups_round2 import (
        ...
        _cluster_sim_raw = False
        _CLUSTER_BACKEND = "python"
        _logger.info("加速模块(cluster): 使用 accelerators.python_fallback.speedups_round2")
