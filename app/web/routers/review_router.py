...
def get_review_data(candidate_id: int) -> dict:
    ...
    with get_session() as db:
        ...
        segment = db.exec(
            __import__("sqlmodel").select(RawSegment).where(
        ...
        if session:
            danmaku_rows = db.exec(
                __import__("sqlmodel").select(Danmaku.ts).where(
        ...
        next_candidates = []
        all_cands = db.exec(
            __import__("sqlmodel").select(HighlightCandidate).where(
...
async def adjust_boundary(
    ...
    with get_session() as db:
        ...
        event = db.exec(
            __import__("sqlmodel").select(HighlightEvent).where(
