...
**版本：V0.1.8.2.1 Alpha** (`0.1.8.2.1-alpha`)
...
| 步骤 | 内容 | 大小 | 说明 |
|------|------|------|------|
| ① | 下载源码 | ~500 KB | 从 GitHub 仓库拉取 `app/` `config/` `pyproject.toml` `requirements-bundle.txt` |
...
> **源码去哪了？** `app/` `config/` `pyproject.toml` `requirements-bundle.txt` 等文件**不在分发目录中**，由 `launcher.exe` 首次运行时自动从 GitHub 仓库的 `main` 分支拉取到分发目录下。这样 PnP 目录始终保持最小体积（仅 ~8 MB），任何时候都能获取到主仓库的最新代码。
