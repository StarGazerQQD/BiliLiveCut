...
**版本：V0.1.12.4b Alpha** (`0.1.12.4b-alpha`)
...
| C 编译器（可选） | — | Visual Studio Build Tools（用于编译 Aho-Corasick 加速模块，无编译器则自动回退纯 Python） |

### C 加速模块（V0.1.9 / V0.1.10 新增）

自 V0.1.9 起, 核心 CPU 热点使用 C 扩展加速（Aho-Corasick 多模式匹配、余弦相似度、字符 bigram）。
V0.1.10 新增剩余 3 个热点（聚类矩阵 O(N²)、弹幕基线分桶、SRT 字幕组装）的 Cython+Python 加速,
**聚类矩阵额外支持 Rust+rayon 并行计算**（有 Rust 环境时 `python build_rust.py` 编译启用,无 Rust 则自动回退）。

- **自动检测**: `pip install -e .` 自动尝试编译; 编译失败 → 自动回退纯 Python 实现
- **C 扩展编译**: 安装 [Visual Studio Build Tools](https://visualstudio.microsoft.com/zh-hans/downloads/)（勾选「C++ 桌面开发」），然后:
  ...
  ```
- **Rust 编译 (可选)**: 安装 [Rust](https://rustup.rs) 后运行 `python build_rust.py`,聚类矩阵可再提速 5–8×
- **启动确认**: 查看日志确认后端 — `加速模块(cluster): Rust+rayon 已加载` / `Cython 已加载` / `使用纯 Python 后备`
- **主要提速**: 关键词匹配 (20-50×)、弹幕情绪 (10-30×)、主题聚类 (5-15× / Rust 30-80×)、弹幕基线 (10-30×)、SRT 组装 (3-8×)

### 多引擎 ASR 流水线（V0.1.12 新增）
...
| 步骤 | 内容 | 大小 | 说明 |
|------|------|------|------|
| ① | 下载源码 | ~500 KB | 从 GitHub 仓库拉取 `app/` `config/` `pyproject.toml` `setup.py` `setup_c.py` `requirements-bundle.txt` |
...
├── build_exe.py                     # PyInstaller 编译脚本（launcher.py → launcher.exe）
├── build_bundle.py                  # 离线打包脚本（下载模型/依赖/FFmpeg/复制源码）
├── pip.ini                          # pip 镜像源配置（阿里云 + 清华备用）
├── .env.example                     # 配置模板（供参考，launcher.exe 自动生成 .env）
├── .gitignore                       # Git 忽略规则
└── README.md                        # 本文件
```

> **源码去哪了？** `app/` `config/` `pyproject.toml` `setup.py` `setup_c.py` `requirements-bundle.txt` 等文件**不在分发目录中**，由 `launcher.exe` 首次运行时自动从 GitHub 仓库的 `main` 分支拉取到分发目录下。这样 PnP 目录始终保持最小体积（仅 ~8 MB），任何时候都能获取到主仓库的最新代码。

### 运行时动态生成（首次启动后）

```
├── app/                     # ★ 主工程源码（launcher.exe 自动从 GitHub 下载）
├── config/                  # ★ 关键词 / 评分 YAML（launcher.exe 自动下载）
├── requirements-bundle.txt  # ★ 依赖清单（launcher.exe 自动下载）
├── pyproject.toml           # ★ 项目配置（launcher.exe 自动下载）
│
...
1. **双击 `launcher.exe`** → 等待自动部署完成（首次联网下载约 10-30 分钟）
...
| 下载源码失败 / 网络错误 | 检查网络连接，确保能访问 `github.com`；重试即可 |
