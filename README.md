...
**V0.1.17.3 不再把五分钟录制分段直接当成一个固定切片。** 控制台按录制场次生成 GMT+8 时间线，在真正的高光时间点展示摘要、1～2 条代表弹幕、置信度和来源依据；点击节点仍可进入详细审核并按动态边界生成视频。整场结束并完成最终分析后，系统还会把所有未拒绝节点按时间先后整理为一段连续的“全场高光总结”，不会按录制分片拆段。
...
- 全场高光总结只消费当前时间线节点，按 GMT+8 从早到晚串联事件；人工纠错、审核或重分析改变时间线后会自动重生成，LLM 暂不可用时保留可读的规则版结果。
...
- Python 包、CLI、C/Cython、Rust、Portable、Docker、GitHub Actions、测试和用户文档当前统一为 `0.1.17.3-alpha`，Engine Pack 兼容区间同步为 `0.1.17.3-alpha ≤ app < 0.1.18`。
...
实时转写页的“重新识别”可修复历史污染文本。操作会删除当前转写和未人工处理、未渲染的自动候选后重新排队；若已有人工审核、确认主题、成片或正在运行的任务，服务端会拒绝覆盖并说明原因。每条转写还会显示源 TS 文件名；需要直接进入 NLE 剪辑时使用“无损导出 MP4”，应用会保留原始编码并把首个有效视频帧校准到 `0s`，避免普通 TS→MP4 重封装因 AAC 预滚产生首帧黑画面。
