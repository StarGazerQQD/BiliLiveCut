...
升级到本版时，现有应用版本严格校验仍会拒绝旧版数据库，即使 Schema 编号同为 5；请保留旧目录，在独立目录创建本版数据库并重新配置。程序不会自动迁移旧数据。版本与构建验证见 [0.1.18.2 验收记录](docs/audit/2026-09-08-v0.1.18.2.md)。
...
- 普通 Windows 用户：[Portable 小白使用说明](packaging/portable/USER_GUIDE_ZH.md)
- Portable 构建、Engine Pack 与发行结构：[即插即用版说明](packaging/portable/README.md)
- Event-first 检测、事件补全与成片评分：[热点检测器说明](docs/hotspot-detector.md)
- C / Cython / Rust 接口、边界与构建：[原生加速模块说明](docs/native-acceleration.md)
- Docker 部署：[Docker 说明](packaging/docker/README.md)
...
- ClipScorer 以完整事件而非单个最大音量峰确定成片分与边界；前后文可跨连续分段，静音吸附只向外扩展，媒体缺口会硬性截断。
...
## V0.1.17 版本总结：场次时间线与可追溯剪辑

**V0.1.17 至 V0.1.17.4 把逐候选列表升级为按主播和录制场次组织的 GMT+8 高光时间线，并集中修复了跨分段边界、审核出片、页面草稿和历史记录问题。** 这一版本线是当前 Event-first 架构的直接业务基础；逐项记录已归档到 [0.1.17 系列 Changelog](docs/changelog/CHANGELOG_PRE_0.1.17.md)。

### V0.1.17：动态高光时间线

- 每个五分钟原始分段可分析多个相互独立的峰值，候选能够跨连续分段读取前后文；动态入点、出点和静音吸附取代固定 1 分 30 秒切片，真实断流缺口始终是硬边界。
- 弹幕按默认 `7.5` 秒接收延迟向前对齐画面，时间线节点保存摘要、1～2 条代表弹幕、来源信号、动态边界和跨分段状态；同场临近且内容相似的结果会去簇。
- 新增房间热词、`错误词=正确词` 学习别名、人工转写回写、审核反馈阈值建议和可恢复的场次重分析；重算时保护人工审核、边界、草稿、成片和人工正文。
- 统一 naive/aware 时间、跨分段相对偏移和多候选事务提交，修复摘要、候选窗口与实际成片错位，以及自动轮询覆盖转写草稿的问题。

### V0.1.17.1～V0.1.17.2：生产状态与页面稳定性

- 非终结审核决策会把任务移出 `awaiting_review`；人工批准独立成片进入带租约和心跳的后台渲染，成功后同步成品 ID。拒绝候选不会继续进入成品队列。
- 活动直播会等待相邻分段补齐后文，同段支持多个候选；断流恢复受 20 次或 180 秒预算限制，停止显示 `stopped`，只有显式暂停显示 `paused`。CUDA 可用时可配置 1～8 个独立 ASR 任务并行。
- 所有自动轮询页面统一保留未保存输入、下拉选择和展开状态；场次时间线内容不变时不重建 DOM，确有更新时恢复滚动位置，不会在阅读摘要或来源评分时跳回顶部。

### V0.1.17.3～V0.1.17.4：整场分析、媒体导出与历史场次

- 录制收尾后按时间顺序汇集同场全部最终 ASR，再交给 LLM 做一次全局分析；它不是分段摘要或高光节点的简单拼接。人工纠错或重分析会使旧总结失效并重建。
- 实时转写显示源 TS 文件名并支持复制和无损 MP4 导出；导出、审片预览、主成片和派生版本统一校正首个有效音视频帧时间轴，避免开头黑帧。
- Whisper 遇到当前后端不支持的显式 CTranslate2 计算类型时只对该兼容性错误改用 `auto` 重试；其他设备和模型错误保持可见。
- “实时转写”和“弹幕热度”可按完整录制历史选择场次；编辑器展开或有草稿时锁定场次切换。每次真正开始录制前会重新查询直播间标题和主播名，失败时保留最近一次有效资料。
...
- 实时转写页同时展示整理正文、片段概括、原始 ASR、实际语音引擎及每条转写对应的源 TS 文件名；可一键复制文件名，方便直接定位原始分段剪辑，后续高光分析默认消费整理后的可读正文。
...
- 高光复核理由、关键词等文本特征只读取音频峰值对应的候选窗口；最终成片强制完整覆盖该分析窗口，LLM 只能向外扩展边界，静音吸附也不会向内切掉理由所描述的事件。审片正文与投稿文案再按最终保存的成片边界裁剪；词级时间戳是当前转写结果的必需数据。
...
- 连续无法恢复取流时当前默认按 20 次或 180 秒双上限自动结束本场录制；预算耗尽后等待一次真实离线，避免房间仍显示直播时立即重启失败；任一上限可单独设为 `0` 禁用，全部禁用会恢复无限重试，不建议用于无人值守场景。
...
- Launcher、Engine Pack、Lite/Full 与 Payload 的 CLI 入口统一使用 UTF-8 stdout/stderr，并为不可编码字符保留回退表示，修复 Windows `cp1252` 控制台崩溃和误报。
...
- Release 标签必须与当前版本真源逐字符一致，并按规范的小写 `-alpha` 标记 GitHub prerelease。
...
- **严格 Schema**: `schema_meta` 元信息表 + SHA-256 指纹只接受当前结构；不一致的数据库拒绝启动
