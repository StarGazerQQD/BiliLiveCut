...
**主程序版本：V0.1.15.3 Alpha** (`0.1.15.3-alpha`)
...
当前高光模型独立发布版本为 **V0.1.15.2 HL-Alpha**。它与主程序/Portable 的 `0.1.15.3-alpha` 分开编号；前者表示模型数据契约、训练流程、注册表和在线适配的版本，后者表示应用运行时和发行包版本。
...
- **固定业务源码**：Payload 从提交 `99c9f16` 通过 `git archive` 提取，不混入构建机工作区内容。
...
| 特性 | 说明 |
|------|------|
| **Source 固定** | 源码来自当前发布基线 `99c9f16`，通过 `git archive` 提取，不混入工作区未提交改动 |
...
控制台采用分组侧栏导航，审核队列、审片工作台和合集编辑器共享统一视觉与交互规范；桌面端强调信息密度和操作反馈，窄屏自动切换为横向导航或单列布局，并提供键盘焦点态与减少动态效果支持。

审片工作台支持入点/出点调整和版本化重新渲染。人工边界会校验最短/最长时长、原始录像范围及中间分片缺口；每次重新渲染保留独立成片文件，不覆盖之前的审核版本。

人工审核入口为 `/review/queue`。通过 `REVIEWER_ACCOUNTS_JSON` 可配置多个审核员独立账号；审核员只能访问审核工作台和审片媒体，不能进入其他管理 API。候选需先领取再修改，领取具有超时自动释放和冲突保护；支持审核草稿、边界/决策撤销、管理员强制接管及 `GET /review/api/audit` 审计查询。`REVIEW_BLIND_MODE=true` 时，普通审核员在提交前看不到模型评分、理由和特征，减少模型意见对人工判断的干扰。

远程部署必须同时设置 `ADMIN_PASSWORD`；审核员账号不能替代管理员安全门禁。账号示例见 `.env.example`，真实密码只应写入未提交的 `.env`。

批准出片、审核重渲染、合集渲染和上传/重试会立即返回后台作业，不再长时间占用 HTTP 请求。作业状态持久化，可在控制台“任务队列”查看进度、错误和结果，也可通过 `GET /api/jobs`、`GET /api/jobs/{job_id}` 查询，通过 `POST /api/jobs/{job_id}/cancel` 或 `/retry` 取消、重试。服务重启会恢复安全可重跑的渲染作业；已开始的上传不会自动重放，避免平台已收件但本地未知时重复投稿。FFmpeg 渲染取消会主动终止当前外部进程，已完成的主片不会被当作损坏文件删除。

录制中的房间支持“高光打点”“停止并收尾”和“强制停止”。普通停止会完成末尾片段登记并暂停自动拉起；恢复录制会创建新会话，明确保留中间时间缺口。高光打点默认保留当前时刻前 20 秒、后 40 秒，并直接进入人工审核候选池。
