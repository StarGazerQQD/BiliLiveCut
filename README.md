...
**当前版本：V0.1.17.4 Alpha** (`0.1.17.4-alpha`)
...
## V0.1.17.4 新特性：场次时间线与可校正分析闭环

**V0.1.17.4 不再把五分钟录制分段直接当成一个固定切片。** 控制台按录制场次生成 GMT+8 时间线，在真正的高光时间点展示摘要、1～2 条代表弹幕、置信度和来源依据；点击节点仍可进入详细审核并按动态边界生成视频。整场结束并完成最终分析后，系统会按时间顺序汇集同场全部最终 ASR，并把这份完整上下文一次性交给 LLM 做全局分析，生成不按录制分片拆段的“全场高光总结”。
...
- 每次真正开始一场新录制前都会重新查询直播间标题和主播名，避免沿用上一场已经变化的标题；查询暂时失败时保留最近一次成功资料，不阻断已授权录制。
- “实时转写”和“弹幕热度”都可用录制场次下拉查看数据库中的完整历史，不再只显示固定数量的最近记录。两个页面分别记住选择；列表内容没有变化时不会重建 DOM 或打断阅读。
- 实时转写、房间词典、房间配置和功能开关在自动刷新期间保护未保存草稿；人工纠错或房间配置编辑器一经展开就暂停对应列表重绘，输入、选择与勾选状态以及保存期间继续产生的新修改都不会被迟到响应覆盖。转写编辑期间还会锁定场次下拉，必须保存或取消后才能切换。原始 ASR、已学习词典和时间线“来源与评分”的展开状态也会保留；时间线确有更新时会恢复阅读位置，避免页面自动跳顶。
