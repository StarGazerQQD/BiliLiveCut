...
**当前版本：V0.1.17.3 Alpha** (`0.1.17.3-alpha`)
...
## V0.1.17.3 新特性：场次时间线与可校正分析闭环

**V0.1.17.3 不再把五分钟录制分段直接当成一个固定切片。** 控制台按录制场次生成 GMT+8 时间线，在真正的高光时间点展示摘要、1～2 条代表弹幕、置信度和来源依据；点击节点仍可进入详细审核并按动态边界生成视频。整场结束并完成最终分析后，系统会按时间顺序汇集同场全部最终 ASR，并把这份完整上下文一次性交给 LLM 做全局分析，生成不按录制分片拆段的“全场高光总结”。

- 每个录制分段最多分析 4 个分散的候选窗口：优先使用音频峰值，并用均匀探针覆盖低声量但有语义的事件；高光可跨越相邻原始分段，入点和出点按语音、弹幕与上下文动态计算，不再固定为 1 分 30 秒。
...
- 全场高光总结消费同场全部最终 ASR，而不是拼接时间线节点或分段摘要；LLM 必须基于完整上下文一次完成全局分析，人工纠错或重分析后会自动失效并重生成，LLM 无有效结果时明确失败并按持久任务策略重试。
...
- 主播下播需连续多次确认并等待收尾延迟；恢复开播会撤销停止。连续取流失败默认在 20 次或 180 秒时结束本场，并等待一次真实离线后才允许下一次开播，避免仍显示直播时反复重建失败会话。
- 实时转写、房间词典、房间配置和功能开关在自动刷新期间保护未保存草稿；人工纠错或房间配置编辑器一经展开就暂停对应列表重绘，输入、选择与勾选状态以及保存期间继续产生的新修改都不会被迟到响应覆盖。原始 ASR、已学习词典和时间线“来源与评分”的展开状态也会保留；时间线确有更新时会恢复阅读位置，避免页面自动跳顶。
- 人工审核后的任务立即离开 `awaiting_review`；明确“独立成片”会进入带租约心跳的渲染队列，渲染完成后同步进入成品切片。
...
- Python 包、CLI、C/Cython、Rust、Portable、Docker、GitHub Actions、测试和用户文档当前统一为 `0.1.17.3-alpha`。Alpha 发行只接受同版本 Runtime、Payload、Engine Pack、配置与数据库，不迁移或兼容旧格式。
...
实时转写页的“重新识别”会删除当前场次可重建的自动分析结果后重新排队；若存在正在运行的任务，服务端会拒绝并说明原因。每条转写还会显示源 TS 文件名；需要直接进入 NLE 剪辑时使用“无损导出 MP4”，应用会保留原始编码并把首个有效视频帧校准到 `0s`，避免普通 TS→MP4 重封装因 AAC 预滚产生首帧黑画面。
