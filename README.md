...
**当前版本：V0.1.17.2 Alpha** (`0.1.17.2-alpha`)
...
## V0.1.17.2 新特性：场次时间线与可校正分析闭环

**V0.1.17.2 不再把五分钟录制分段直接当成一个固定切片。** 控制台按录制场次生成 GMT+8 时间线，在真正的高光时间点展示摘要、1～2 条代表弹幕、置信度和来源依据；点击节点仍可进入详细审核并按动态边界生成视频。
...
- 实时转写、房间词典、房间配置和功能开关在自动刷新期间保护未保存草稿；转写纠错、原始 ASR、已学习词典和时间线“来源与评分”的展开状态也会保留。时间线轮询不会重复重建未变化内容，确有更新时会恢复阅读位置，避免页面自动跳顶。
...
- Python 包、CLI、C/Cython、Rust、Portable、Docker、GitHub Actions、测试和用户文档当前统一为 `0.1.17.2-alpha`，Engine Pack 兼容区间同步为 `0.1.17.2-alpha ≤ app < 0.1.18`。
